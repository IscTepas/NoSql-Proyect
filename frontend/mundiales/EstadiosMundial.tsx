    ...
    <Card className="group hover:shadow-xl hover:shadow-[var(--wc-gold)]/10 transition-all duration-300 hover:-translate-y-1 overflow-hidden border-[var(--wc-gold)]/10 bg-white">
      <CardContent className="p-0">
        <div className="relative h-32 bg-gradient-to-br from-[var(--wc-gold)]/[0.04] via-transparent to-[var(--wc-black)]/[0.02] overflow-hidden">
          <div className="absolute inset-0 flex items-center justify-center"><Building2 className="h-16 w-16 text-[var(--wc-gold)]/15" /></div>
        ...
        <div className="p-4 space-y-3">
          <h3 className="font-semibold text-sm leading-tight group-hover:text-[var(--wc-gold-dark)] transition-colors">{stadium.nombre}</h3>
          ...
          <div className="flex items-center justify-between">
            <Badge variant="outline" className="text-[10px] border-[var(--wc-gold)]/20"><Clock className="h-3 w-3 mr-1" />{stadium.zonaHoraria || "UTC"}</Badge>
            <Badge variant="outline" className="text-[10px] border-[var(--wc-gold)]/20"><Globe className="h-3 w-3 mr-1" />{PAIS_LABELS[countryCode] || countryCode}</Badge>
          </div>
          {matchCount > 0 && (
            <button onClick={(e) => { e.stopPropagation(); onToggle(); }} className="w-full flex items-center justify-between text-xs text-[var(--wc-gold-dark)] hover:text-[var(--wc-gold)] transition-colors pt-2 border-t border-[var(--wc-gold)]/10">
        ...
        {expanded && partidos.length > 0 && (
          <div className="px-4 pb-4 space-y-2">
            {partidos.map((p) => {
              ...
              return (
                <div key={p._id} className="rounded-lg border border-[var(--wc-gold)]/10 p-2.5 hover:shadow-sm transition-all duration-200">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex flex-col items-center mx-2 shrink-0">
                      {played ? (<span className="text-[11px] font-bold bg-[var(--wc-gold)] text-white px-2 py-0.5 rounded">{p.marcador.ft[0]} - {p.marcador.ft[1]}</span>) : (<span className="text-[10px] text-muted-foreground font-mono">{p.hora?.split(" ")[0]}</span>)}
                    </div>
    ...
    <div className="container mx-auto px-4 py-8 space-y-6">
      <div>
        <h1 className="text-2xl font-black tracking-tight flex items-center gap-2 text-[var(--wc-black)]">
          <div className="h-8 w-8 rounded-lg bg-[var(--wc-gold)] flex items-center justify-center"><MapPin className="h-4 w-4 text-white" /></div>
          Estadios
        </h1>
      ...
      <div className="relative max-w-md">
        ...
        <Input placeholder="Buscar por nombre o ciudad..." value={busqueda} onChange={(e) => setBusqueda(e.target.value)} className="pl-9 border-[var(--wc-gold)]/20 focus-visible:ring-[var(--wc-gold)]/30" />
      </div>
      <div className="flex flex-wrap gap-2">
        {PAISES.map((p) => (
          <button key={p.value} onClick={() => setPaisFilter(p.value)} className={`px-3.5 py-1.5 rounded-full text-xs font-medium transition-all duration-200 ${paisFilter === p.value ? "bg-[var(--wc-gold)] text-white shadow-md shadow-[var(--wc-gold)]/20" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>
