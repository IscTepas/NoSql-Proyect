...
function MatchCard({ partido, expanded, onToggle }: { partido: Partido; expanded: boolean; onToggle: () => void }) {
  ...
  return (
    <div className="relative flex gap-4">
      ...
      <Card
        className={`flex-1 mb-4 hover:shadow-lg hover:shadow-[var(--wc-gold)]/5 transition-all duration-300 border-[var(--wc-gold)]/10 cursor-pointer overflow-hidden bg-white ${
          played ? "border-l-2" : ""
        }`}
        style={played ? { borderLeftColor: "var(--wc-green)" } : undefined}
        onClick={onToggle}
      >
        <CardContent className="p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <Badge className={`${colors.bg} ${colors.text} text-[10px] border-0`}>
                {partido.ronda}
              </Badge>
              {grupoNombre && (
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                ...
                <span>{partido.hora?.split(" ")[0]}</span>
              </div>
              {expanded ? (
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex flex-col items-center mx-4 shrink-0">
              {played ? (
                <div className="flex items-center gap-2">
                  <span className={`text-xl font-bold ${partido.marcador.ft[0] > partido.marcador.ft[1] ? "text-[var(--wc-green)]" : ""}`}>
                    {partido.marcador.ft[0]}
                  </span>
                  <span className="text-muted-foreground">-</span>
                  <span className={`text-xl font-bold ${partido.marcador.ft[1] > partido.marcador.ft[0] ? "text-[var(--wc-green)]" : ""}`}>
                    {partido.marcador.ft[1]}
                  </span>
                </div>
              ) : (
                <Badge variant="outline" className="text-xs font-mono border-[var(--wc-gold)]/20">
                  VS
                </Badge>
              )}
            </div>
            ...
            <div className="flex items-center gap-3 min-w-0 flex-1 justify-end">
              <div className="text-right min-w-0">
                <p className="text-sm font-semibold truncate">{partido.equipo2?.nombre || "Por definir"}</p>
                <p className="text-[10px] text-muted-foreground">{partido.equipo2?.fifaCode}</p>
              </div>
          ...
          {partido.estadio && (
            <div className="flex items-center gap-1.5 mt-3 text-xs text-muted-foreground">
              ...
              <span>{partido.estadio.nombre}, {partido.estadio.ciudad}</span>
            </div>
          )}
        </CardContent>
          ...
          <div className="border-t border-[var(--wc-gold)]/10 p-4 space-y-3 bg-[var(--wc-gold)]/[0.02]">
              ...
              <div className="space-y-2">
                {partido.marcador.ht && partido.marcador.ht.length === 2 && (
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
                    ...
                    <span className="font-mono font-medium">{partido.marcador.ht[0]} - {partido.marcador.ht[1]}</span>
                  </div>
                )}
                {partido.marcador.et && partido.marcador.et.length === 2 && (
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
                    <span>Tiempo extra:</span>
                    <span className="font-mono font-medium">{partido.marcador.et[0]} - {partido.marcador.et[1]}</span>
                  </div>
                )}
                {partido.marcador.p && partido.marcador.p.length === 2 && (
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
                    <span>Penales:</span>
                    <span className="font-mono font-medium">{partido.marcador.p[0]} - {partido.marcador.p[1]}</span>
                  </div>
                )}
              </div>
            )}
                ...
                <div className="space-y-1">
                  {partido.goles1?.map((g, i) => (
                    <div key={`g1-${i}`} className="flex items-center gap-2 text-xs">
                      <Circle className="h-2 w-2 fill-[var(--wc-green)] text-[var(--wc-green)]" />
                      <span className="font-medium">{g.nombre}</span>
                      <span className="text-muted-foreground">{g.minuto}&apos;</span>
                      {g.penalti && <Badge className="bg-[var(--wc-gold)]/10 text-[var(--wc-gold-dark)] text-[8px] border-0">PEN</Badge>}
                      {g.autogol && <Badge className="bg-[var(--wc-red)] text-white text-[8px] border-0">OG</Badge>}
                    </div>
                  ))}
                  {partido.goles2?.map((g, i) => (
                    <div key={`g2-${i}`} className="flex items-center gap-2 text-xs">
                      <Circle className="h-2 w-2 fill-[var(--wc-orange)] text-[var(--wc-orange)]" />
                      <span className="font-medium">{g.nombre}</span>
                      <span className="text-muted-foreground">{g.minuto}&apos;</span>
                      {g.penalti && <Badge className="bg-[var(--wc-gold)]/10 text-[var(--wc-gold-dark)] text-[8px] border-0">PEN</Badge>}
                      {g.autogol && <Badge className="bg-[var(--wc-red)] text-white text-[8px] border-0">OG</Badge>}
            ...
            {partido.equipoGanador && (
              <div className="flex items-center gap-2 text-xs">
                ...
                <Badge className="bg-[var(--wc-green)] text-white text-[10px] border-0">
                  <Flag code={partido.equipoGanador.fifaCode} size={12} className="mr-1" />
                  {partido.equipoGanador.nombre}
                </Badge>
              </div>
            )}
          </div>
...
export default function PartidosPage() {
  ...
  const rondaCount = useMemo(() => {
    ...
    partidos.forEach((p) => {
      counts[p.ronda] = (counts[p.ronda] || 0) + 1;
    });
    return counts;
  }, [partidos]);
  ...
  const filtered = useMemo(() => {
    ...
    return partidos
      .filter((p) => {
        ...
        const grupoMatch = `group ${grupoNombre}`.includes(searchLower) || grupoNombre.toLowerCase().includes(searchLower);
        return (
          grupoMatch ||
          p.equipo1?.nombre?.toLowerCase().includes(searchLower) ||
          p.equipo1?.fifaCode?.toLowerCase().includes(searchLower) ||
          p.equipo2?.nombre?.toLowerCase().includes(searchLower) ||
          p.equipo2?.fifaCode?.toLowerCase().includes(searchLower) ||
          p.estadio?.nombre?.toLowerCase().includes(searchLower) ||
          p.estadio?.ciudad?.toLowerCase().includes(searchLower) ||
          p.fecha?.toLowerCase().includes(searchLower) ||
          p.ronda?.toLowerCase().includes(searchLower)
        );
      })
    ...
    <div className="container mx-auto px-4 py-8 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          {filtered.length} de {partidos?.length ?? 0} partidos
        </p>
      </div>
      ...
      <div className="relative max-w-md">
        ...
        <Input
          placeholder="Buscar por equipo, grupo, estadio o fecha..."
          value={busqueda}
      ...
      <div className="flex flex-wrap gap-2">
        {RONDAS.map((r) => (
          <button
            key={r.value}
            onClick={() => setRondaFilter(r.value)}
          ...
          >
            {r.label}
            {r.value !== "all" && rondaCount[r.value] ? (
              <span className="ml-1 opacity-70">({rondaCount[r.value]})</span>
            ) : r.value === "Matchday 1" || r.value === "Matchday 2" || r.value === "Matchday 3" ? (
              <span className="ml-1 opacity-70">
                ({(rondaCount["Matchday 1"] || 0) + (rondaCount["Matchday 2"] || 0) + (rondaCount["Matchday 3"] || 0)})
              </span>
            ) : null}
          </button>
        ))}
      ...
      <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
        {[
          { label: "Grupos", color: "bg-blue-500" },
          { label: "Round of 32", color: "bg-orange-500" },
          { label: "Round of 16", color: "bg-purple-500" },
          { label: "Quarter-final", color: "bg-amber-500" },
          { label: "Semi-final", color: "bg-rose-500" },
          { label: "3rd Place", color: "bg-red-500" },
          { label: "Final", color: "bg-[var(--wc-gold)]" },
        ].map((item) => (
        ...
        {Object.entries(groupedByDate).map(([fecha, partidosFecha]) => (
          <div key={fecha} className="space-y-2">
            <div className="flex items-center gap-3 sticky top-20 z-20 bg-background/95 backdrop-blur-sm py-2">
              ...
              <Badge variant="secondary" className="text-xs font-semibold px-3 py-1 bg-[var(--wc-gold)] text-white border-0">
                <Calendar className="h-3.5 w-3.5 mr-1.5" />
                {fecha}
                <span className="ml-2 opacity-60">({partidosFecha.length})</span>
              </Badge>
              <div className="h-px flex-1 bg-[var(--wc-gold)]/10" />
            </div>
            {partidosFecha.map((p) => (
      ...
      {filtered.length === 0 && (
        <div className="text-center py-16">
          ...
          <p className="text-muted-foreground">No se encontraron partidos con esos filtros</p>
        </div>
      )}
    </div>
  );
